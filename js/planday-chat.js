/** PlanDay chatbot - file:// + Live Server + GitHub Pages compatible */
(function () {
/** 챗봇 상수 (10-4 리팩토링) */
const CHAT_SUGGESTIONS = ["오늘 일정", "남은 일정", "주말 일정", "추천해줘", "도움말"];
const CHAT_SUGGESTIONS_GPT = ["어제 일정 알려줘", "이번 주 바쁜 날?", "너는 누구야?", "내일 3시에 스터디 추가"];
const GEMINI_KEY_STORAGE = "planDayGeminiKey";
const GPT_MODE_ENABLED_STORAGE = "planDayGptModeEnabled";
const CHAT_TONE_STORAGE = "planDayChatTone";
const CHAT_CUSTOM_PROMPT_STORAGE = "planDayChatCustomPrompt";
const ACTIVE_USER_PROMPT_KEY = "planDayActiveUserPromptId";
const GEMINI_MODEL = "gemini-3.6-flash";
const GEMINI_EDGE_FUNCTION = "gemini-chat";

const USER_PROMPT_TYPE_LABELS = {
  custom: "커스텀",
  roleplay: "역할 놀이",
  coach: "코치/멘토",
  study: "학습/연습"
...
function bindChatDeps(nextDeps) {
  deps = nextDeps;
}
...
function getSessionUsageFromMessages(messages) {
  ...
  for (const m of list) {
    if (m.role === "assistant" && m.usage?.model) {
      ...
      totalCost += Number(m.usage.estimated_cost_usd) || 0;
    }
  }
  return { aiReplies, totalTokens, totalCost };
}
...
function formatMessageUsageLabel(usageMeta) {
  ...
  const cost = formatUsdSmall(usageMeta.estimated_cost_usd);
  return `${usageMeta.model} · 텍스트 in ${inTok} · out ${outTok} · ${cost}`;
}
...
function appendChatMessage(text, role = "bot", usageMeta = null) {
  ...
  if (usageMeta?.model) {
    ...
    el.title = "텍스트 API 사용량 (PlanDay는 오디오 API 미사용)";
...
function updateChatGptModeUi() {
  ...
  if (ruleHint) {
    ruleHint.hidden = isGptModeActive();
  }
...
function getChatCapabilitiesReply() {
  ...
  if (gpt) {
    lines.push("", "✨ AI 모드 ON (Gemini) — 더 자유로운 대화 가능");
  } else {
    lines.push("", "💡 AI 모드는 AI설정에서 (Gemini API Key 필요 · 무료 할당량)");
  }
  return lines.join("\n");
}
...
function getChatFallbackReply(noGpt) {
  return [
    "음, 잘 이해하지 못했어요. 이렇게 말해 보세요:",
    "• \"내일 일정\" / \"다음 일정\" / \"D-day\"",
    "• \"내일 3시에 스터디 추가\"",
    "• \"팀 미팅 삭제\" / \"수업 완료\"",
    "• \"달력 열어\" / \"중요 일정\"",
    "",
    "\"도움말\"이라고 하시면 할 수 있는 일을 전부 알려드릴게요."
  ].join("\n");
}
...
async function handleRuleBasedChat(msg) {
  ...
  if (window.PlanDayChatRules) {
    const advanced = await window.PlanDayChatRules.handleRuleBasedMessage(getChatDeps(), {
      parseDateFromChat,
      parseChatDate,
      parseChatTime,
      tryHandleCasualChat,
      getChatScheduleContext
    }, msg);
...
async function sendChatMessage(text) {
  ...
  try {
    ...
    if (result.usage && isGptModeActive()) {
      ...
      const modelName = modelMatch?.[1]?.replace(/\([^)]*\)$/, "") || GEMINI_MODEL;
    ...
    } else if (usageMeta && !chatState.currentSessionId) {
      getChatDeps().showToast?.("대화 DB 없음 — 사용량은 화면에만 표시됩니다");
    }
  } catch (err) {
    typing.remove();
    console.error(err);
...
function createChatbot(appDeps) {
  ...
  window.createPlanDayChatbot = createChatbot;
})();
